...
function volumeSpec(name, vol) {
  ...
  case 'empty_dir':
    ...
    break;
  default:
    throw new Error(`vol_type ${vol_type} not supported`)
  }
...
function volumes(volumeMap) {
  ...
  for (const [name, spec] of Object.entries(volumeMap)) {
    volumes.push(volumeSpec(name, spec));
  }
  return {
    spec: { volumes }
  };
...
function affinities(affinityList) {
...
function envVars(envs) {
  ...
  for (var e of envs) {
    ...
    if (field === undefined) {
      ...
      switch (kind) {
      case 'config':
        from.configMapRef = ref;
        break;
      case 'secret':
        from.secretRef = ref;
        break;
      default:
        throw new ArgumentError(`kind for envVar of ${kind} not supported`);
      }
      envFrom.push(from);
      continue;
    }
...
const action = {
  ...
  net: _args => { throw new Error(`net actions not implemented yet`); },
};
...
function ports(pp) {
  function containerPort(p) {
    ...
    const [ _, protocol, hostIP, hostPort, containerPort ] = spec.match(portRe);
    const port = {
      name,
  ...
  return { ports: pp.map(containerPort) };
}
